...
const gainDisplayElement = document.getElementById(
  'gain-display',
) as HTMLSpanElement
...
let activeMidiInputDevice: any = null
  ; (async () => {
    ...
    try {
      ...
      const midiOutDropdown = document.getElementById(
        'midi-out',
      ) as HTMLSelectElement
      const midiInDropdown = document.getElementById(
        'midi-in',
      ) as HTMLSelectElement
      ...
      if (eventType === 'note_on') {
        ...
        activeNotes.forEach((timeSec, noteNum) => {
          if (currentPianoTimeSec - timeSec > MAX_NOTE_DURATION_SECONDS) {
            console.info(
              `Note ${noteNum} has been active for ${currentPianoTimeSec -
              timeSec}, ` +
